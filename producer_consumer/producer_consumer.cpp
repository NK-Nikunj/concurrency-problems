...
void producer()
{
    while(!should_end)
    {
        ...
        std::cout << "Producer produced item with string: "
...
void consumer()
{
    while(!should_end)
    {
        ...
        std::cout << "Consumer consumed item with string: "
                  << item.get_content() << std::endl;
...
void simulate(int p, int c)
{
    ...
    for(auto i = 0; i < p; ++i)
    {
        std::unique_ptr<hpx::thread> pt = 
                    std::make_unique<hpx::thread>(&producer);
    ...
    {
        std::unique_ptr<hpx::thread> pt = 
                    std::make_unique<hpx::thread>(&consumer);
